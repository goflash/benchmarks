...
// A Fiber v3 server: GET /ping -> "pong"
func main() {
	app := fiber.New()
	...
	app.Get("/context", func(c fiber.Ctx) error {
		...
		// Read the value back
		if v := c.Locals("test-key"); v != nil {
			return c.SendString(v.(string))
		}
		return c.SendString("context-ok")
	})
	...
	app.Get("/wildcard/*", func(c fiber.Ctx) error { return c.SendString(c.Params("*")) })
	...
	cmw := app.Group("/mw10")
	for i := 0; i < 10; i++ {
		cmw.Use(func(c fiber.Ctx) error { return c.Next() })
	}
	cmw.Get("/ping", func(c fiber.Ctx) error { return c.SendString("pong") })

	log.Fatal(app.Listen(":17785"))
}
